...
const QuickActions = () => {
  ...
  const lastSessionText = lastSession
    ? `${lastSession.language.charAt(0).toUpperCase() + lastSession.language.slice(1)} - ${lastSession.scenario.charAt(0).toUpperCase() + lastSession.scenario.slice(1)}`
    : "No recent session";
    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <button
          ...
          className="bg-[#1A1F2E] border border-white/10 rounded-2xl p-6 hover:border-[#6C47FF] hover:-translate-y-1 transition-all text-left h-40 flex flex-col justify-between group"
        >
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-[#E8ECEF]/60">{lastSessionText}</p>
          </div>
        </button>
